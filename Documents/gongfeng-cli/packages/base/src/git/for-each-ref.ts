...
import { Branch, BranchType, IBranchTip, ITrackingBranch, CommitIdentity } from '../models';
import { createForEachRefParser } from './git-delimiter-parser';

/** Get all the branches. */
export async function getBranches(repoPath: string, ...prefixes: string[]): Promise<ReadonlyArray<Branch>> {
  ...
  const result = await git(['for-each-ref', ...formatArgs, ...prefixes], repoPath, 'getBranches', {
 ...
 * forwarded.
 *
 * @param repoPath Repository path to get the branches from.
 */
export async function getBranchesDifferingFromUpstream(repoPath: string): Promise<ReadonlyArray<ITrackingBranch>> {
  const { formatArgs, parse } = createForEachRefParser({
    fullName: '%(refname)',
    sha: '%(objectname)', // SHA
    upstream: '%(upstream)',
  ...
  const result = await git(['for-each-ref', ...formatArgs, ...prefixes], repoPath, 'getBranchesDifferingFromUpstream', {
    expectedErrors: new Set([GitError.NotAGitRepository]),
  });
