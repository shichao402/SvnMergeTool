...
import { Branch, BranchType, IAheadBehind, CommitOneLine } from '../models';
...
export async function getAheadBehind(repoPath: string, range: string): Promise<IAheadBehind | null> {
  ...
  const result = await git(args, repoPath, 'getAheadBehind', {
...
export async function getBranchAheadBehind(repoPath: string, branch: Branch): Promise<IAheadBehind | null> {
  ...
  return getAheadBehind(repoPath, range);
}
...
export async function getCommitsBetweenCommits(
  ...
  return getCommitsInRange(repoPath, range);
}
 ...
 */
export async function getCommitsInRange(repoPath: string, range: string): Promise<ReadonlyArray<CommitOneLine> | null> {
  const args = [
    'rev-list',
    range,
    '--reverse',
  ...
  const result = await git(args, repoPath, 'getCommitsInRange', options);
