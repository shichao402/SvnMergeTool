...
import { BranchConfig } from '../models/branch';
...
export function getConfigValue(
  ...
  return getConfigValueInPath(names, repoPath, onlyLocal);
}
...
export function getGlobalConfigValue(
  ...
  return getConfigValueInPath(names, null, false, undefined, env);
}
...
export async function getGlobalBooleanConfigValue(
  ...
  const values = await getConfigValueInPath(names, null, false, 'bool', env);
  if (values?.length) {
...
export async function setConfigValue(
  ...
  return setConfigValueInPath(name, value, repoPath, env);
}
...
/** Set the global config value by name. */
export async function addGlobalConfigValue(name: string, value: string): Promise<void> {
...
/** Set the global config value by name. */
export async function addGlobalConfigValueIfMissing(name: string, value: string): Promise<void> {
...
export async function removeConfigValue(
  ...
  return removeConfigValueInPath(name, repoPath, env);
}
...
export async function getBranchMergeConfig(repoPath: string, branch: string): Promise<BranchConfig | null> {
  const results = await getConfigValue(repoPath, ['--get-regexp', `^branch.${branch}.(remote|merge)$`]);
  if (!results) {
    return null;
  }
