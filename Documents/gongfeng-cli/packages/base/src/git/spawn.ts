...
import { IGitSpawnExecutionOptions } from '@tencent/code-dugite/build/lib/git-process';
 ...
 *                         returned.
 */
export const spawnGit = (args: string[], path: string, name: string, options?: IGitSpawnExecutionOptions) => {
  return GitProcess.spawn(args, path, {
    ...options,
    env: { ...options?.env },
  });
};
  ...
  return new Promise<ProcessOutput>(async (resolve, reject) => {
    ...
    if (process.stdout) {
      process.stdout.on('data', (chunk: Buffer) => {
        ...
        if (stdOutMaxLength && totalStdoutLength >= stdOutMaxLength && !killSignalSent) {
          process.kill();
          killSignalSent = true;
        }
      });
    ...
    process.on('close', (code, signal) => {
      const stdout = Buffer.concat(
        stdoutChunks,
        stdOutMaxLength ? Math.min(stdOutMaxLength, totalStdoutLength) : totalStdoutLength,
      );
      ...
      if ((code !== null && exitCodes.has(code)) || signal) {
        ...
        reject(
          new Error(`Git returned an unexpected exit code '${code}' which should be handled by the caller (${name}).'`),
        );
      }
    });
  });
