...
import { tailByLine } from '../common/file-system';
...
export async function executionOptionsWithProgress(
  ...
  if (options.trackLFSProgress) {
    try {
      ...
      console.log('Error writing LFS progress file', e);
      env = { GIT_LFS_PROGRESS: null };
    }
  }

  return merge(options, {
    processCallback: createProgressProcessCallback(parser, lfsProgressPath, progressCallback),
