...
const debug = Debug('gongfeng:git-status');
 ...
 */
export async function getStatus(repoPath: string): Promise<IStatusResult | null> {
  const args = ['--no-optional-locks', 'status', '--untracked-files=all', '--branch', '--porcelain=2', '-z'];

  const result = await spawnAndComplete(args, repoPath, 'getStatus', new Set([0, 128]));

  if (result.exitCode === 128) {
    debug(`'git status' returned 128 for '${repoPath}' and is likely missing its .git directory`);
    return null;
  }
  ...
  const mergeHeadFound = await isMergeHeadSet(repoPath);
  const conflictedFilesInIndex = entries.some((e) => conflictStatusCodes.indexOf(e.statusCode) > -1);
  const rebaseInternalState = await getRebaseInternalState(repoPath);
  ...
  const isCherryPickingHeadFound = await isCherryPickHeadFound(repoPath);

  const squashMsgFound = await isSquashMsgSet(repoPath);
...
async function getMergeConflictDetails(repoPath: string) {
  const conflictCountsByPath = await getFilesWithConflictMarkers(repoPath);
  const binaryFilePaths = await getBinaryPaths(repoPath, 'MERGE_HEAD');
  return {
...
async function getRebaseConflictDetails(repoPath: string) {
  const conflictCountsByPath = await getFilesWithConflictMarkers(repoPath);
  const binaryFilePaths = await getBinaryPaths(repoPath, 'REBASE_HEAD');
  return {
    conflictCountsByPath,
    binaryFilePaths,
  };
 ...
 */
async function getWorkingDirectoryConflictDetails(repoPath: string) {
  const conflictCountsByPath = await getFilesWithConflictMarkers(repoPath);
  let binaryFilePaths: ReadonlyArray<string> = [];
  try {
    // its totally fine if HEAD doesn't exist, which throws an error
    binaryFilePaths = await getBinaryPaths(repoPath, 'HEAD');
  } catch (error) {}
 ...
 *
 * @param repoPath to get details from
...
async function getConflictDetails(
  ...
  try {
    if (mergeHeadFound) {
      return await getMergeConflictDetails(repoPath);
    }

    if (rebaseInternalState !== null) {
      return await getRebaseConflictDetails(repoPath);
    }

    if (lookForStashConflicts) {
      return await getWorkingDirectoryConflictDetails(repoPath);
    }
  } catch (error) {
    console.log('Unexpected error from git operations in getConflictDetails', error);
  }
  return {
