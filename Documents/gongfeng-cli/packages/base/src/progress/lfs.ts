import { getTempFilePath } from '../common/file-system';
...
export async function createLFSProgressFile(): Promise<string> {
  const path = await getTempFilePath('GongFengCli-lfs-progress');
...
export class GitLFSProgressParser {
  ...
  public parse(line: string): IGitProgress | IGitOutput {
    ...
    if (isNaN(estimatedFileCount) || isNaN(fileTransferred) || isNaN(fileSize)) {
      return { kind: 'context', percent: 0, text: line };
    }
    ...
    const transferProgress = `${formatBytes(totalTransferred, 1)} / ${formatBytes(totalEstimated, 1)}`;
