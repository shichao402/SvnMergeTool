 ...
 * disposed of.
 */
export function tailByLine(path: string, cb: (line: string) => void): Disposable {
  const tailer = new Tailer(path);

  const onErrorDisposable = tailer.onError((error: any) => {
    // log.warn(`Unable to tail path: ${path}`, error);
    console.log(`Unable to tail path: ${path}`, error);
  });
 ...
 *              file to read to
 */
export async function readPartialFile(path: string, start: number, end: number): Promise<Buffer> {
...
export async function getFileHash(path: string, type: 'sha1' | 'sha256'): Promise<string> {
