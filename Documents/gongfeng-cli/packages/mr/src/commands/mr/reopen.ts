...
const debug = Debug('gongfeng-mr:reopen');

/**
 * 重新打开合并请求命令
 */
export default class Reopen extends BaseCommand {
  static summary = '重新打开一个合并请求';
  static description = `可以通过 iid 或者源分支指定需要重新打开的合并请求
  当通过源分支指定时默认重新打开源分支最新的合并请求`;
  static examples = ['gf mr reopen 42', 'gf mr reopen dev'];
  static usage = 'mr reopen <iidOrBranch> [flags]';
  ...
  async run(): Promise<void> {
    ...
    const mergeRequest4Detail = await this.apiService.getMergeRequest4DetailByIidOrBranch(
      projectId,
      branch,
    ...
    if (mergeRequest.state === MergeRequestState.OPENED || mergeRequest.state === MergeRequestState.REOPENED) {
      console.log(
        color.warn(__('mergeRequestAlreadyOpen', { iid: `${mergeRequest.iid}`, title: mergeRequest.titleRaw })),
      );
      this.exit(0);
      return;
    }
    ...
    if (mergeRequest.state === MergeRequestState.MERGED) {
      console.log(color.warn(__('mergedReopenError', { iid: `${mergeRequest.iid}`, title: mergeRequest.titleRaw })));
      this.exit(0);
      return;
    }
    ...
    const isSucceed = await this.apiService.reopenMergeRequest(projectId, mergeRequest.iid);
    if (isSucceed) {
      console.log(
        color.success(__('openedMergeRequest', { iid: `${mergeRequest.iid}`, title: mergeRequest.titleRaw })),
      );
    } else {
      console.log(
        color.error(__('reopenMergeRequestError', { iid: `${mergeRequest.iid}`, title: mergeRequest.titleRaw })),
      );
    }
  }
}
