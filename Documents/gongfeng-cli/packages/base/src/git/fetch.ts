import { IFetchProgress } from '../models/progress';
import { git, IGitExecutionOptions, IGitResult } from './core';
import { Remote } from '../models/remote';
import { executionOptionsWithProgress } from '../progress/from-process';
import { FetchProgressParser } from '../progress/fetch';
import { ITrackingBranch } from '../models/branch';
import { ChildProcess } from 'child_process';
import Debug from 'debug';

const debug = Debug('gongfeng:fetch');

async function getFetchArgs(
  repoPath: string,
  remote: string,
  enableRecurseSubmodules: boolean,
  progressCallback?: (progress: IFetchProgress) => void,
) {
  if (enableRecurseSubmodules) {
    return progressCallback !== null
      ? ['fetch', '--progress', '--prune', '--recurse-submodules=on-demand', remote]
      : ['fetch', '--prune', '--recurse-submodules=on-demand', remote];
  }
  return progressCallback !== null ? ['fetch', '--progress', '--prune', remote] : ['fetch', '--prune', remote];
}

/**
 * Fetch from the given remote.
 *
 * @param repoPath - The repository to fetch into
 *
 * @param remote     - The remote to fetch from
 *
 * @param enableRecurseSubmodules   - Whether enable submodules
 *
 * @param progressCallback - An optional function which will be invoked
 *                           with information about the current progress
 *                           of the fetch operation. When provided this enables
 *                           the '--progress' command line flag for
 *                           'git fetch'.
 */
export async function fetch(
  repoPath: string,
  remote: Remote,
  enableRecurseSubmodules: boolean,
  progressCallback?: (progress: IFetchProgress) => void,
): Promise<void> {
  let opts: IGitExecutionOptions = {
    successExitCodes: new Set([0]),
  };

  if (progressCallback) {
    const title = `Fetching ${remote.name}`;
    const kind = 'fetch';

    opts = await executionOptionsWithProgress(
      { ...opts, trackLFSProgress: true },
      new FetchProgressParser(),
      (progress) => {
        // In addition to progress output from the remote end and from
        // git itself, the stderr output from pull contains information
        // about ref updates. We don't need to bring those into the progress
        // stream so we'll just punt on anything we don't know about for now.
        if (progress.kind === 'context') {
          if (!progress.text.startsWith('remote: Counting objects')) {
            return;
          }
        }

        const description = progress.kind === 'progress' ? progress.details.text : progress.text;
        const value = progress.percent;

        progressCallback({
          kind,
          title,
          description,
          value,
          remote: remote.name,
        });
      },
    );

    // Initial progress
    progressCallback({ kind, title, value: 0, remote: remote.name });
  }

  const args = await getFetchArgs(repoPath, remote.name, enableRecurseSubmodules, progressCallback);

  await git(args, repoPath, 'fetch', opts);
}

/** Fetch a given refspec from the given remote. */
export async function fetchRefspec(
  repoPath: string,
  remote: string,
  refspec: string,
  processCallback?: (process: ChildProcess) => void,
): Promise<IGitResult | null> {
  try {
    return await git(['fetch', remote, refspec], repoPath, 'fetchRefspec', {
      successExitCodes: new Set([0, 128]),
      processCallback,
    });
  } catch (e) {
    debug('git fetch refspec failed');
    return null;
  }
}

export async function fastForwardBranches(repoPath: string, branches: ReadonlyArray<ITrackingBranch>): Promise<void> {
  if (branches.length === 0) {
    return;
  }

  const refPairs = branches.map((branch) => `${branch.upstreamRef}:${branch.ref}`);

  const opts: IGitExecutionOptions = {
    // Fetch exits with an exit code of 1 if one or more refs failed to update
    // which is what we expect will happen
    successExitCodes: new Set([0, 1]),
    env: {
      // This will make sure the reflog entries are correct after
      // fast-forwarding the branches.
      GIT_REFLOG_ACTION: 'pull',
    },
    stdin: refPairs.join('\n'),
  };

  await git(
    [
      'fetch',
      '.',
      // Make sure we don't try to update branches that can't be fast-forwarded
      // even if the user disabled this via the git config option
      // `fetch.showForcedUpdates`
      '--show-forced-updates',
      // Prevent `git fetch` from touching the `FETCH_HEAD`
      '--no-write-fetch-head',
      // Take branch refs from stdin to circumvent shell max line length
      // limitations (mainly on Windows)
      '--stdin',
    ],
    repoPath,
    'fastForwardBranches',
    opts,
  );
}
