...
enum ActionType {
  COPILOT_SH_CANCEL = 'copilot_sh_cancel',
  COPILOT_SH_COPY = 'copilot_sh_copy',
  COPILOT_SH_EDIT = 'copilot_sh_edit',
}
export default class Sh extends BaseCommand {
  static summary = 'Shell AI 命令行 Copilot';
  static examples = ['gf copilot sh "列出js文件"'];
  ...
  async run(): Promise<void> {
    const { args } = await this.parse(Sh);
    const { question } = args;
    ...
    this.llmClient.client.interceptors.request.use((config: any) => {
    ...
    // 设置 cli 全局的系统规则
    const systemPrompt = `你的任务是基于用户提出的自然语言问题，仅用一行shell命令来给出答案。请确保你的回答仅包含一个shell命令，不要添加任何额外的文字说明或其他内容。以下是一个示例：
    - **问题**：如何列出当前目录下所有的js文件？
    - **回答**：find . -name "*.js"
    
    如果用户需要对命令进行修改或进一步询问，你需要继续提供仅包含单个shell命令的回答，遵循上述规则。
    任务开始了，问题：`;
  ...
  private async loopAI(conversation: Conversation, questions: string[], currentCommand: string) {
    ...
    if (answer.confirm === CANCEL) {
      this.actionReport.report(ActionType.COPILOT_SH_CANCEL);
      this.exit(0);
    }
    if (answer.confirm === EDIT) {
      ...
      if (answer.question) {
        ...
        this.actionReport.report(ActionType.COPILOT_SH_EDIT);
        await this.loopAI(conversation, questions, command);
      }
    }
    if (answer.confirm === COPY) {
      clipboardy.writeSync(currentCommand);
      this.actionReport.report(ActionType.COPILOT_SH_COPY);
  ...
  private async explainFromAI(command: string) {
    ...
    const systemPrompt =
      '作为Shell命令的解释大师，您的任务是向用户解释他们提出的Shell命令。请确保您的回答既精确又直接，避免冗长的解释，请务必保持在200字以内，另外，回答可以适当分段保持良好的可读性。现在，用户向您询问的Shell命令是：';
    // 由于混元的问题，暂时不使用 systemPrompt，以 userPrompt 的形式代替
    // conversation.addGlobalSystemMessage(systemPrompt);
    const turn = new Turn();
