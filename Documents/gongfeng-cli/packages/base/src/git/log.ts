...
import { forceUnwrap } from '../common';

const SubmoduleFileMode = '160000';

function mapSubmoduleStatusFileModes(status: string, srcMode: string, dstMode: string): SubmoduleStatus | undefined {
  return srcMode === SubmoduleFileMode && dstMode === SubmoduleFileMode && status === 'M'
    ...
    : (srcMode === SubmoduleFileMode && status === 'D') || (dstMode === SubmoduleFileMode && status === 'A')
    ? {
        commitChanged: false,
        untrackedChanges: false,
        modifiedChanges: false,
...
export async function getCommits(
  ...
  const result = await git(args, repoPath, 'getCommits', {
...
export async function getChangedFiles(repoPath: string, sha: string): Promise<IChangesetData> {
  ...
  const { stdout } = await git(args, repoPath, 'getChangesFiles');
...
export function parseRawLogWithNumstat(stdout: string, sha: string, parentCommitish: string) {
  ...
  for (let i = 0; i < lines.length - 1; i++) {
    ...
    if (line.startsWith(':')) {
      ...
      const status = forceUnwrap('Invalid log output (status)', lineComponents[lineComponents.length - 1]);
      // eslint-disable-next-line no-plusplus
      const oldPath = /^R|C/.test(status) ? forceUnwrap('Missing old path', lines[++i]) : undefined;

      // eslint-disable-next-line no-plusplus
      const path = forceUnwrap('Missing path', lines[++i]);
...
export async function getCommit(repoPath: string, ref: string): Promise<Commit | null> {
  const commits = await getCommits(repoPath, ref, 1);
  if (commits.length < 1) {
    return null;
  }
