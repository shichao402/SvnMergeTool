import { FileEntry, GitStatusEntry, SubmoduleStatus, UnmergedEntrySummary } from '../models/status';
import Debug from 'debug';

const debug = Debug('gongfeng:status-parser');
...
function parseChangedEntry(field: string): IStatusEntry {
  ...
  if (!match) {
    debug(`parseChangedEntry parse error: ${field}`);
...
function parsedRenamedOrCopiedEntry(field: string, oldPath: string | undefined): IStatusEntry {
  ...
  if (!match) {
    debug(`parsedRenamedOrCopiedEntry parse error: ${field}`);
...
function parseUnmergedEntry(field: string): IStatusEntry {
  ...
  if (!match) {
    debug(`parseUnmergedEntry parse error: ${field}`);
 ...
 */
export function mapStatus(statusCode: string, submoduleStatusCode: string): FileEntry {
