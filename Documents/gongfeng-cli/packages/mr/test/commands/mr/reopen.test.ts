...
describe('mr reopen', () => {
  ...
  describe('with merge request', () => {
    ...
    // 通过 mr iid 重新打开 mr (state === opened)
    test
      .stub(base, 'checkAuth', () => {
        return () => {
          return true;
      ...
      .nock('https://git.woa.com', (api) => {
        ...
        api.get(`/api/web/v1/projects/1000/merge_requests/${mockMergeRequest.iid}`).reply(200, {
          mergeRequest: { ...mockMergeRequest, state: 'opened' },
        });
      })
      .stdout()
      .command(['mr reopen', `${mockMergeRequest.iid}`])
      .exit(0)
      .it('mr is already open (state is opened)', (ctx) => {
        expect(ctx.stdout).to.contain(
          `Merge request !${mockMergeRequest.iid} (${mockMergeRequest.titleRaw}) is already open`,
        );
      });

    // 通过 mr iid 重新打开 mr (state === reopened)
    test
      .stub(base, 'checkAuth', () => {
        return () => {
          return true;
      ...
      .nock('https://git.woa.com', (api) => {
        ...
        // 模拟根据 mr iid 获取 mr 信息
        api.get(`/api/web/v1/projects/1000/merge_requests/${mockMergeRequest.iid}`).reply(200, {
          mergeRequest: { ...mockMergeRequest, state: 'reopened' },
        });
      })
      .stdout()
      .command(['mr reopen', `${mockMergeRequest.iid}`])
      .exit(0)
      .it('mr is already open (state is reopened)', (ctx) => {
        expect(ctx.stdout).to.contain(
          `Merge request !${mockMergeRequest.iid} (${mockMergeRequest.titleRaw}) is already open`,
        );
      });

    // 通过 mr iid 重新打开 mr (state === merged)
    test
      .stub(base, 'checkAuth', () => {
        return () => {
          return true;
      ...
      .command(['mr reopen', `${mockMergeRequest.iid}`])
      .exit(0)
      .it('mr was already merged', (ctx) => {
        expect(ctx.stdout).to.contain(
          `Merge request !${mockMergeRequest.iid} (${mockMergeRequest.titleRaw}) can't be reopened because it was already merged`,
        );
      });
    ...
    test
      ...
      .nock('https://git.woa.com', (api) => {
        ...
        api.put(`/api/web/v1/projects/1000/merge_requests/${mockMergeRequest.iid}/reopen`).reply(200);
      })
      .stdout()
      .command(['mr reopen', branch, '-c "reopen with comment"'])
      .it('mr reopened with comment', (ctx) => {
        expect(ctx.stdout).to.contain('Failed to add a comment');
        expect(ctx.stdout).to.contain(`Reopened merge request !${mockMergeRequest.iid} (${mockMergeRequest.titleRaw})`);
      });
    ...
    test
      ...
      .nock('https://git.woa.com', (api) => {
        ...
        api.put(`/api/web/v1/projects/1000/merge_requests/${mockMergeRequest.iid}/reopen`).reply(500);
      })
      .stdout()
      .command(['mr reopen', `${mockMergeRequest.iid}`])
      .it('fail to reopen', (ctx) => {
        expect(ctx.stdout).to.contain(
          `Failed to reopen merge request !${mockMergeRequest.iid} (${mockMergeRequest.titleRaw})`,
        );
      });
  });
});
