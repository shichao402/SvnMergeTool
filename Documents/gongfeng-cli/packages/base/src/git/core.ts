...
} from '@tencent/code-dugite';
import { ChildProcess } from 'child_process';
import { Readable } from 'stream';
import * as split2 from 'split2';
import { assertNever } from '../common/fatal-error';
import Debug from 'debug';
import * as os from 'os';
import { default as findGit, Git } from 'find-git-exec';
import { dirname } from 'path';

const debug = Debug('gongfeng:git');

let localGit: Git | undefined = undefined;
...
export async function git(
  ...
  debug(`command: ${commandName} in path: ${path}`);

  // 非windows 和 macos 平台使用系统自带的 git
  debug(`os: ${os.platform()}`);
  if (os.platform() !== 'win32' && os.platform() !== 'darwin') {
    debug('use system git');
    try {
      localGit = await findGit();
    } catch {}

    if (localGit?.path && localGit.execPath) {
      debug('system git founded');
      const { path, execPath } = localGit;
...
function getDescriptionForError(error: DugiteError): string | null {
  if (isAuthFailureError(error)) {
    return 'Authentication failed. You are not logged in to your account';
  }
  ...
  switch (error) {
    ...
    case DugiteError.DefaultBranchDeletionFailed:
      return 'The branch is the repository default branch and cannot be deleted.';
    ...
    case DugiteError.PushWithFileSizeExceedingLimit:
      return 'The push operation includes a file which exceeds GitHub file size restriction of 100MB. Please remove the file from history and try again.';
